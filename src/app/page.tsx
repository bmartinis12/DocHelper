    ...
    <>
      <MaxWidthWrapper className="mb-12 mt-28 sm:mt-14 flex flex-col items-center justify-center text-center">
        <div className="mx-auto mb-4 flex max-w-fit items-center justify-center space-x-2 overflow-hidden rounded-full border border-gray-200 bg-white px-7 py-2 shadow-md backdrop-blur transition-all hover:border-gray-300 hover:bg-white/50">
          <p className="text-sm font-semibold text-gray-700">DocHelper is now public!</p>
        </div>
        ...
        <p className="mt-5 max-w-prose text-zinc-700 sm:text-xl">DocHelper allows you to have conversations with any PDF document. Simply upload your file and start asking questions right away.</p>
        {!user || !user.id ? (
          <LoginLink className={buttonVariants({ size: 'lg', className: 'mt-5' })}>
            Get Started <ArrowRight className="ml-2 h-5 w-5" />
          </LoginLink>
        ) : (
          <Link className={buttonVariants({ size: 'lg', className: 'mt-5' })} href='/dashboard'>
            Head to the Dashboard <ArrowRight className="ml-2 h-5 w-5" />
          </Link>
        )}
      </MaxWidthWrapper >
      <div>
        <div className="relative isolate">
          <div aria-hidden='true' className="pointer-events-none absolute inset-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-80">
            ...
            <div className="mx-auto max-w-6xl px-6 lg:px-8">
              <div className="mt-16 flow-root sm:mt-24">
                <div className="-m-2 rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
                  <Image src='/dashboard-preview.jpg' width={1364} height={866} quality={100} alt="product preview" priority className="rounded-md bg-white p-2 sm:p-8 md:p-20 ring-1 ring-gray-900/10" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto mb-32 mt-32 max-w-5xl px-2 sm:mt-56">
        <div className="mb-12 px-6 lg:px-8">
          <div className="mx-auto max-w-2xl sm:text-center">
            <h2 className="mt-2 font-bold text-4xl text-gray-900 sm:text-5xl">Start chatting in minutes</h2>
            <p className="mt-4 text-lg text-gray-600">Chatting with your PDF files has never been easier than with Quil.</p>
          </div>
        </div>
        <ol className="my-8 space-y-4 pt-8 md:flex md:space-x-12 md:space-y-0">
          ...
          <li className="md:flex-1">
            <div className="flex flex-col space-y-2 border-l-4 border-zinc-300 py-2 pl-4 md:border-l-0 md:border-t-2 md:pb-0 md:pl-0 md:pt-4">
              ...
              <span className="mt-2 text-zinc-700">It&apos;s that simple. Try out DocHelper today - it really takes less than a minute</span>
            </div>
          </li>
        </ol>
        <div className="mx-auto max-w-6xl px-6 lg:px-8">
          <div className="mt-16 flow-root sm:mt-24">
            <div className="-m-2 rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4">
              <Image src='/file-upload-preview.jpg' width={1419} height={732} quality={100} alt="uploading preview" priority className="rounded-md bg-white p-2 sm:p-8 md:p-20 ring-1 ring-gray-900/10" />
            </div>
          </div>
        </div>
      </div>
